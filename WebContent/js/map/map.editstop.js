...
function getMap() {
	//map
	var map = L.map('map_edit_stop').setView([city_center_lat, city_center_lon], 16);
	window.map = map;
    	...
    	window.geocoder.reverse(e.latlng, window.map.options.crs.scale(window.map.getZoom()), function(results) {
			...
			if (r) {
				...
				var content = "<div><h3 align='center'>New Stop</h3>" +
					...
					"<tr><td>Action</td><td><a href='javascript:new_from_map(" + lat + "," + lon + ")'>New</a>&nbsp;/&nbsp;<a href='javascript:remove_marker(" + marker.options.id + ")'>Cancel</a></td></tr></table></div>";
...
function onMarkerDragEnd(e) {
	...
	window.geocoder.reverse(e.target._latlng, window.map.options.crs.scale(window.map.getZoom()), function(results) {
		...
		if (r) {
			...
			"<tr><td>Location</td><td>(" + lat + ", " + lon + ")</td></tr>" + 
			"<tr><td>Description</td><td>" + r.name + "</td></tr>" + 
			"<tr><td>Action</td><td><a href='javascript:new_from_map(" + lat + "," + lon + ")'>New</a>&nbsp;/&nbsp;<a href='javascript:remove_marker(" + e.target.options.id + ")'>Cancel</a></td></tr></table></div>";
