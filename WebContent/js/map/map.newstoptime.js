...
function getMap() {
	...
	var map = L.map("map_new_stop_time").setView([city_center_lat, city_center_lon], 16);
	//set title layer
...
function getPopupContent(marker) {
	...
	var popup_content = "<div><h3 align='center'>Stop Information</h3>" +
	"<table class='reference'><tr><th>Field</th><th>Content</th></tr>" +
	"<tr><td>Stop ID</td><td>" + marker.options.id + "</td></tr>" +
	"<tr><td>Name</td><td>" + marker.options.name + "</td></tr>" +
	"<tr><td>Sequence *</td><td><input id='map_stop_sequence' type='number' value='" + sequence + "' style='width:90%' class='validate_stop_sequence' onchange='move_map_seq_to_new()'></td></tr>" + 
	"<tr><td>Arrival time *</td><td><input id='map_arrival_time' type='time' value='" + marker.options.arrival_time + "' style='width:90%' onchange='move_map_at_to_new()'></td>" +
	"<tr><td>Departure time *</td><td><input id='map_departure_time' type='time' value='" + marker.options.departure_time + "' style='width:90%' onchange='move_map_dt_to_new()'></td>" +
	"<tr><td>Action</td><td><a href='javascript:btn_save()'>Save</a> / <a href='javascript:change_to_tab_new()'>Add more details</a></td>" +
	"</table></div>";
...
function onMarkerClick(e) {
	...
	//set stop_id info to 'new' tab
	$('#new_stop_id').val(e.target.options.id);
	$('#new_stop_sequence').val(e.target.options.sequence);	
...
function draw_shapes_of_trip() {
	...
	var shape_id = $('#new_route_id').val() + "##" + service_id;
	//set the central point of map into city center
