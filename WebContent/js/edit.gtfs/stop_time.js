...
//set current_tab to "map" or "edit" or "list", distinguish which is the current tab opened
var current_tab = "#map";
...
function validate_trip_id() {
	var tagid_trip_id = "#edit_trip_id";
...
function validate_stop_id() {
	var tagid_stop_id = "#edit_stop_id";
...
function validate_stop_sequence() {
	var tagid_stop_sequence = "#edit_stop_sequence";
...
function validate_arrival_time() {
	var tagid_arrival_time = "#edit_arrival_time";
...
function validate_departure_time() {
	var tagid_departure_time = "#edit_departure_time";
...
function validate_shape_dist_traveled() {
	var tagid_shape_dist_traveled = "#edit_shape_dist_traveled";
...
function stop_time_to_json() {
	var stop_time_json = '{';
	stop_time_json += '"trip_id":"' + $("#edit_trip_id").val() + '",';
	stop_time_json += '"agency_id":"' + agency_id + '",';
	stop_time_json += '"stop_id":"' + $("#edit_stop_id").val() + '",';
	stop_time_json += '"stop_sequence":"' + $("#edit_stop_sequence").val() + '",';
	stop_time_json += '"arrival_time":"' + $("#edit_arrival_time").val() + '",';
	stop_time_json += '"departure_time":"' + $("#edit_departure_time").val() + '",';
	stop_time_json += '"pickup_type":"' + $("#edit_pickup_type").val() + '",';
	stop_time_json += '"drop_off_type":"' + $("#edit_drop_off_type").val() + '",';
	stop_time_json += '"stop_headsign":"' + $("#edit_stop_headsign").val() + '",';
	stop_time_json += '"shape_dist_traveled":"' + $("#edit_shape_dist_traveled").val() + '"}';
	return stop_time_json;
}

//copy the value of stop_sequence of map tab to edit tab
function move_map_seq_to_edit() {
	$('#edit_stop_sequence').val($('#map_stop_sequence').val());
}

//copy the value of arrival_time of map tab to edit tab
function move_map_at_to_edit() {
	$('#edit_arrival_time').val($('#map_arrival_time').val());
}

//copy the value of departure_time of map tab to edit tab
function move_map_dt_to_edit() {
	$('#edit_departure_time').val($('#map_departure_time').val());
}
...
function btn_reset() {
	if(current_tab == "#edit") {
		$("#edit_stop_sequence").val('');
		$("#edit_arrival_time").val('');
		$("#edit_departure_time").val('');
		$("#edit_pickup_type").val('0').trigger('chosen:updated');
		$("#edit_drop_off_type").val('0').trigger('chosen:updated');
		$("#edit_stop_headsign").val('');
		$("#edit_shape_dist_traveled").val('');
	} else if(current_tab == "#list") {
		$("#list_route_id").val('').trigger('chosen:updated');
		$("#list_trip_id").val('').trigger('chosen:updated');
		document.getElementById("table_list_stop_time").innerHTML = "";
	} else {//current_tab == "map"
		$("#map_stop_sequence").val('');
		$("#map_arrival_time").val('');
		$("#map_departure_time").val('');
		$("#edit_stop_sequence").val('');
		$("#edit_arrival_time").val('');
		$("#edit_departure_time").val('');
		$("#edit_pickup_type").val('0').trigger('chosen:updated');
...
function load_stop_time(trip_id) {
	if(trip_id == null || trip_id == "") return;
	$.ajax({
		...
		success: function (data) {
			var list = stringToJson(data);
			//填充stop_time内容在marker的popup中, 并且将icon置为绿色表示已存在该stop_time
			$.each(list.StopTime, function(i, stop_time) {
				for(var i=0; i<stopsArray.length; i++) {
					if(stop_time.stop_id == stopsArray[i].options.id) {
						stopsArray[i].setIcon(StopIconGreen);
						stopsArray[i].options.sequence = stop_time.stop_sequence;
						stopsArray[i].options.arrival_time = stop_time.arrival_time;
						stopsArray[i].options.departure_time = stop_time.departure_time;
						break;
					}
				}
			});
