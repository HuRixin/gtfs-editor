...
<title>Public Transit Open Data Edit Platform</title>
	...
	<script src="js/edit.gtfs/stop_time.js"></script>
...
<body>
	...
	<section id="main" class="column">
		...
		<header><h3 class="tabs_involved">Edit StopTime  <a href="javascript:show_detail_stop_time(&quot;usage&quot;)">(Usage)</a></h3>
		<ul class="tabs">			
		...
		<div class="tab_container">
			<div id="map" class="tab_content" style="position:relative;">
				<div id="map_stop_time" style="width:100%; height:550px;"><script src="js/map/map.editstoptime.js"></script></div>
			...
			<div id="edit" class="tab_content" style="overflow:auto; height:550px">
				...
				<fieldset style="width:48%; float:left;"> <!-- to make two field float next to one another, adjust values accordingly -->
					...
					<div style="margin-left:10px;">
					<select id="edit_drop_off_type" data-placeholder="Choose a Drop off Type..." class="edit_choose">
				...
				<fieldset style="width:48%; float:left;  margin-left: 1%; margin-right: 1%;"> <!-- to make two field float next to one another, adjust values accordingly -->
					...
					<input id="edit_stop_headsign" type="text" style="width:92%;" value="">
				</fieldset>
				<fieldset style="width:48%; float:left;"> <!-- to make two field float next to one another, adjust values accordingly -->
					<label>Shape Traveled Distance <a href="javascript:show_detail_stop_time(&quot;shape_dist_traveled&quot;)">(Read More)</a></label>
					<input id="edit_shape_dist_traveled" type="text" class="validate_shape_dist_traveled" style="width:92%;" value="">
				</fieldset>
				<div class="clear"></div>
			...
			<fieldset style="width:95%; float:left;  margin-left: 2%; margin-right: 2%;">
					...
					<select id="list_route_id" data-placeholder="Choose a Route ID..." class="edit_choose">
						<option value=""></option>
					</select>
					</div>
					<div style="width: 40%; float:left; margin-left:10px;">
					<select id="list_trip_id" data-placeholder="Choose a Trip ID..." class="edit_choose">
						<option value=""></option>
					</select>
					</div>
...
var agency_id = "<%=agency_id%>";
	...
	$(".edit_choose").chosen({disable_search_threshold: 1000, allow_single_deselect: true});
	
	$("#map_route_id").on('change', function(evt, params) {
		if(params === undefined) {	//when user clears the selection (select nothing)
			$('#edit_route_id').val('');	//clear route_id field in 'edit' tab
			...
			$('#edit_route_id').val(params.selected);
			load_trips_of_route(params.selected);
	    }
		$('#edit_trip_id').val('');	//clear trip_id field in 'edit' tab
		$('#edit_stop_id').val('');	//clear stop_id field in 'edit' tab
		$('#edit_stop_sequence').val('');	//clear stop_sequence field in 'edit' tab
		$('#edit_arrival_time').val('');	//clear arrival_time field in 'edit' tab
		$('#edit_departure_time').val('');	//clear departure_time field in 'edit' tab
	}); 
	...
	$("#map_trip_id").on('change', function(evt, params) {
		...
		if(params === undefined) {	//when user clears the selection (select nothing)
			...
			$('#edit_trip_id').val(trip_id);	//set value for trip_id field in 'edit' tab
	    }
		stop_sequence = 0;	//reset stop sequence
		draw_stops_of_trip(trip_id);
		draw_shapes_of_trip();	//get shapes for drawing line of this trip
		load_stop_time(trip_id);
		$('#edit_stop_id').val('');	//clear stop_id field in 'edit' tab
		$('#edit_stop_sequence').val('');	//clear stop_sequence field in 'edit' tab
		$('#edit_arrival_time').val('');	//clear arrival_time field in 'edit' tab
		$('#edit_departure_time').val('');	//clear departure_time field in 'edit' tab
